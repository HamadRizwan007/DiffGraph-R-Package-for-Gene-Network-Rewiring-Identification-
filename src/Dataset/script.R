...
if (length(args) < 3) {
  stop("Usage: Rscript script.R <output_directory> <value> -c <covType>")
}

# Extract output directory, value, and covariance type
...
# Print output directory, value, and covariance type for verification
print(output_directory)
print(value)
print(covType)

# Load the dataset (assuming it's passed as an argument)
data_file <- args[5]
print(data_file)
data(TCGA.BRCA)
# Extract relevant data from the loaded dataset
X <- TCGA.BRCA$X[1,]

#1. Identify differential network between breast cancer subtypes using Dtrace with covType
dtrace.results <- Dtrace(X, value, covType = covType)
